...
static double hyp2f0 ( double, double, double, int, double * );
...
double cephes_special_functions_hyperg( double a, double b, double x)
{
...
if( fabs(temp) < 0.001 * fabs(a) )
	return( exp(x) * cephes_special_functions_hyperg( temp, b, -x )  );
...
if( b > 0 )
	{
	temp = cephes_special_functions_lgam(b);
	t += temp;
	u += temp;
	}
...
temp = exp(u) / cephes_special_functions_gamma(b-a);
h1 *= temp;
err1 *= temp;
...
if( a < 0 )
	temp = exp(t) / cephes_special_functions_gamma(a);
else
	temp = exp( t - cephes_special_functions_lgam(a) );
...
if( b < 0 )
	{
	temp = cephes_special_functions_gamma(b);
	asum *= temp;
	acanc *= fabs(temp);
	}
