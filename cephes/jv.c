...
extern int cephes_special_functions_airy ( double, double *, double *, double *, double * );
extern double cephes_special_functions_polevl ( double, double coef[], int );
extern double cephes_special_functions_j0 ( double );
extern double cephes_special_functions_j1 ( double );
extern double cephes_special_functions_gamma ( double );
extern double cephes_special_functions_lgam ( double );
...
double cephes_special_functions_jv( double n, double x )
{
double k, q, t, y, an;
int i, sign, nint;
...
if( y == an )
	...
	if( n == 1.0 )
		return( sign * cephes_special_functions_j1(x) );
	}
 ...
 */
	if( nint != 0 )
		...
		if( k == 0.0 )
			{
			y = cephes_special_functions_j0(x)/q;
			goto done;
			}
		if( k == 1.0 )
			{
			y = cephes_special_functions_j1(x)/q;
			goto done;
			}
		}
...
if(  (ex > -1023)
  ...
  && (n < (MAXGAM-1.0)) )
	{
	t = pow( 0.5*x, n ) / cephes_special_functions_gamma( n + 1.0 );
#if DEBUG
printf( "pow(.5*x, %.4e)/cephes_special_functions_gamma(n+1)=%.5e\n", n, t );
#endif
	y *= t;
	}
else
...
#if DEBUG
	z = n * log(0.5*x);
	k = cephes_special_functions_lgam( n+1.0 );
	t = z - k;
	printf( "log pow=%.5e, lgam(%.4e)=%.5e\n", z, n+1.0, k );
#else
	t = n * log(0.5*x) - cephes_special_functions_lgam(n + 1.0);
#endif
	if( y < 0 )
		{
		sgngam = -sgngam;
...
#endif
cephes_special_functions_airy( t, &ai, &aip, &bi, &bip );

/* polynomials in expansion */
u[0] = 1.0;
zzi = 1.0/zz;
u[1] = cephes_special_functions_polevl( zzi, P1, 1 )/sz;
u[2] = cephes_special_functions_polevl( zzi, P2, 2 )/zz;
u[3] = cephes_special_functions_polevl( zzi, P3, 3 )/(sz*zz);
pp = zz*zz;
u[4] = cephes_special_functions_polevl( zzi, P4, 4 )/pp;
u[5] = cephes_special_functions_polevl( zzi, P5, 5 )/(pp*sz);
pp *= zz;
u[6] = cephes_special_functions_polevl( zzi, P6, 6 )/pp;
u[7] = cephes_special_functions_polevl( zzi, P7, 7 )/(pp*sz);
...
/* Airy function */
zz = -cbtwo * z;
cephes_special_functions_airy( zz, &ai, &aip, &bi, &bip );
...
F[0] = 1.0;
F[1] = -z/5.0;
F[2] = cephes_special_functions_polevl( z3, PF2, 1 ) * zz;
F[3] = cephes_special_functions_polevl( z3, PF3, 2 );
F[4] = cephes_special_functions_polevl( z3, PF4, 3 ) * z;
G[0] = 0.3 * zz;
G[1] = cephes_special_functions_polevl( z3, PG1, 1 );
G[2] = cephes_special_functions_polevl( z3, PG2, 2 ) * z;
G[3] = cephes_special_functions_polevl( z3, PG3, 2 ) * zz;
#if DEBUG
for( k=0; k<=4; k++ )
	printf( "F[%d] = %.5E\n", k, F[k] );
for( k=0; k<=3; k++ )
