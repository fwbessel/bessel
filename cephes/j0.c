...
extern double cephes_special_functions_p1evl ( double, double coef[], int );
double cephes_special_functions_j0 ( double );

extern double TWOOPI, SQ2OPI, PIO4;

#include <math.h>

double cephes_special_functions_j0(double x)
{
double w, z, p, q, xn;
...
if( x <= 5.0 )
	...
	p = (z - DR1) * (z - DR2);
	p = p * cephes_special_functions_polevl( z, RP, 3)/cephes_special_functions_p1evl( z, RQ, 8 );
	return( p );
	}

w = 5.0/x;
q = 25.0/(x*x);
p = cephes_special_functions_polevl( q, PP, 6)/cephes_special_functions_polevl( q, PQ, 6 );
q = cephes_special_functions_polevl( q, QP, 7)/cephes_special_functions_p1evl( q, QQ, 7 );
xn = x - PIO4;
...
double cephes_special_functions_y0(double x)
{
double w, z, p, q, xn;
...
if( x <= 5.0 )
	...
	z = x * x;
	w = cephes_special_functions_polevl( z, YP, 7) / cephes_special_functions_p1evl( z, YQ, 7 );
	w += TWOOPI * log(x) * cephes_special_functions_j0(x);
	return( w );
	}

w = 5.0/x;
z = 25.0 / (x * x);
p = cephes_special_functions_polevl( z, PP, 6)/cephes_special_functions_polevl( z, PQ, 6 );
q = cephes_special_functions_polevl( z, QP, 7)/cephes_special_functions_p1evl( z, QQ, 7 );
xn = x - PIO4;
