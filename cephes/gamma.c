...
extern int cephes_special_functions_isnan ( double );
extern int cephes_special_functions_isfinite ( double );
...
w = 1.0/x;
w = 1.0 + w * cephes_special_functions_polevl( w, STIR, 4 );
y = exp(x);
if( x > MAXSTIR )
...
double cephes_special_functions_gamma(double x)
{
double p, q, z;
int i;
...
x -= 2.0;
p = cephes_special_functions_polevl( x, P, 6 );
q = cephes_special_functions_polevl( x, Q, 7 );
return( z * p / q );
...
double cephes_special_functions_lgam(double x)
{
double p, q, u, w, z;
int i;
...
sgngam = 1;
#ifdef NANS
if( cephes_special_functions_isnan(x) )
	return(x);
#endif

#ifdef INFINITIES
if( !cephes_special_functions_isfinite(x) )
	return(INFINITY);
#endif
...
if( x < -34.0 )
	{
	q = -x;
	w = cephes_special_functions_lgam(q); /* note this modifies sgngam! */
	p = floor(q);
	if( p == q )
		{
lgsing:
...
if( x < 13.0 )
	...
	p -= 2.0;
	x = x + p;
	p = x * cephes_special_functions_polevl( x, B, 5 ) / cephes_special_functions_p1evl( x, C, 6);
	return( log(z) + p );
	}
...
if( x >= 1000.0 )
	...
	q += cephes_special_functions_polevl( p, A, 4 ) / x;
return( q );
}
