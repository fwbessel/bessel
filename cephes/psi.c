 ...
 *              dx
 *
 * is the logarithmic derivative of the cephes_special_functions_gamma function.
 * For integer x,
 *                   n-1
 *                    -
 * psi(n) = -EUL  +   >  1/k.
...
double cephes_special_functions_psi(double x)
{
...
if( s < 1.0e17 )
	{
	z = 1.0/(s * s);
	y = z * cephes_special_functions_polevl( z, A, 6 );
	}
else
	y = 0.0;
