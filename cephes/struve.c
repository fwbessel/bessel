...
extern double cephes_special_functions_yn ( int, double );
extern double cephes_special_functions_jv ( double, double );
static double yv ( double, double );
static double onef2 (double, double, double, double, double * );
static double threef0 (double, double, double, double, double * );
...
double cephes_special_functions_struve( double v, double x )
{
double y, ya, f, g, h, t;
...
if( (v < 0) && ( v-f == 0.5 ) )
	{
	y = cephes_special_functions_jv( -v, x );
	f = 1.0 - f;
	g =  2.0 * floor(f/2.0);
	if( g != f )
		y = -y;
...
if( onef2err <= threef0err )
	{
	g = cephes_special_functions_gamma( v + 1.5 );
	y = y * h * t / ( 0.5 * f * g );
	return(y);
	}
else
	{
	g = cephes_special_functions_gamma( v + 0.5 );
	ya = ya * h / ( f * g );
	ya = ya + yv( v, x );
	return(ya);
	}
...
if( y == v )
	{
	n = v;
	y = cephes_special_functions_yn( n, x );
	return( y );
	}
t = PI * v;
y = (cos(t) * cephes_special_functions_jv( v, x ) - cephes_special_functions_jv( -v, x ))/sin(t);
return( y );
}
