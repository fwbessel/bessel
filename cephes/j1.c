...
double cephes_special_functions_j1(double x)
{
double w, z, p, q, xn;
...
if( w <= 5.0 )
	{
	z = x * x;	
	w = cephes_special_functions_polevl( z, RP, 3 ) / cephes_special_functions_p1evl( z, RQ, 8 );
	w = w * x * (z - Z1) * (z - Z2);
	return( w );
	}
...
double cephes_special_functions_y1(double x)
{
double w, z, p, q, xn;
...
if( x <= 5.0 )
	...
	z = x * x;
	w = x * (cephes_special_functions_polevl( z, YP, 5 ) / cephes_special_functions_p1evl( z, YQ, 8 ));
	w += TWOOPI * ( cephes_special_functions_j1(x) * log(x)  -  1.0/x );
	return( w );
	}
